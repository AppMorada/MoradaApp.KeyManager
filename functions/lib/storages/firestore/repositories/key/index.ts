...
export class FirestoreKeyRepo implements KeyRepo {
	constructor(@inject(firestoreServiceId) private readonly firestoreService: FirestoreService) {}

	private readonly secretCollection = this.firestoreService.instance.collection('secrets')

	async create(input: Key): Promise<void> {
		const { name, ...rest } = FirestoreKeyMapper.toFlat(input)
		await this.secretCollection.doc(name).create({ ...rest })
			.catch((err) => {
	...
	async remove(id: string): Promise<void> {
		await this.secretCollection.doc(id).delete()
			.catch((err) => {
				throw new FirestoreCustomError({
	...
	async updateAllExpiredKeys(): Promise<void> {
		...
		const refs = await this.secretCollection.where('renewTime', '<=', Date.now())
			.get()
			.catch((err) => {
				throw new FirestoreCustomError({
		...
		refs.forEach((item) => {
			...
			const key = FirestoreKeyMapper.fromFlatToClass({ name: item.id, ...parsedKey })
